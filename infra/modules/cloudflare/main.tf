...
# Dormant until cloudflare_google_client_id/secret are set — kept wired in
# so switching to Google sign-in later is just filling in the Doppler secrets.
resource "cloudflare_zero_trust_access_identity_provider" "google" {
  count      = var.cloudflare_google_client_id != "" ? 1 : 0
  account_id = var.cloudflare_account_id
  name       = "Google"
  type       = "google"
  config = {
